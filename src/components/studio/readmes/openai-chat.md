# Catalyst Connector: OpenAI Chat

Send chat completion requests to OpenAI's GPT models from your Camunda 7 process.

## Quick Start

1. Apply the **Catalyst - OpenAI Chat** template to any Service Task
...
This connector uses an OpenAI API key.

**Getting a key:**
1. Go to [platform.openai.com](https://platform.openai.com)
2. Navigate to **API Keys**
3. Click **Create new secret key**, copy it immediately (starts with `sk-...`)
4. Ensure you have credits loaded under **Billing**
...
|---|---|---|
| Environment Variables (default) | Secret name, e.g. `openai_api_key` | Resolved from `OPENAI_API_KEY` env var on the Camunda engine |
| Docker Secrets (files) | Secret name, e.g. `openai_api_key` | Read from `/run/secrets/openai_api_key` |
| Properties File | Secret name, e.g. `openai_api_key` | Looked up in the properties file on the engine |
| Process Variable (dev only) | Literal key, e.g. `sk-proj-abc123` | Used as-is. **Not recommended** -- the key is visible in Camunda process history. |
...
| **OpenAI API Key** | Authentication | The secret name (for env/file/properties) or literal key (for variable mode) |
| **Request Payload** | Request Configuration | The JSON body sent to OpenAI (see below) |
...
| Field | Value | Purpose |
|-------|-------|---------|
| catalyst_url | `https://api.openai.com/v1/chat/completions` | API endpoint |
...
|-------|------|------------|
| `model` | Literal | Change to `gpt-4o`, `gpt-3.5-turbo`, etc. |
...
Maps fields from OpenAI's response into process variables. Default:
...
|-------------|----------------|-------|------------|
| 401 | `AUTH_FAILED` | Invalid or expired API key | Check your key, ensure credits are loaded |
| 429 | `RATE_LIMITED` | Too many requests | Auto-retried 3 times with backoff. If still failing, wait or upgrade your OpenAI plan |
...
```
[Start] → [User Task: Ask Question] → [Service Task: Ask OpenAI] → [User Task: Show Answer] → [End]
```

1. **User Task** sets `userQuestion` via a form field
2. **Service Task** uses the OpenAI template with payload:
   ```json
   {
     "model": "gpt-4o-mini",
     "messages": [
...
- **Start with `gpt-4o-mini`** — it's fast and cheap. Switch to `gpt-4o` only if you need higher quality.
- **Set temperature low (0.1–0.3)** for factual tasks, higher (0.7–1.0) for creative tasks.
- **Keep `max_tokens` reasonable** — 1000 tokens is roughly 750 words. Don't set it to 10000 "just in case" — you'll pay for what you use.
- **Use the timeout** — complex prompts can take 30+ seconds. The default 60s is usually enough.
